...
| 状態 | 持つ部品 | 書く部品 | 読む部品 |
|---|---|---|---|
| 選択中のシリーズ（`selectedSeriesId`） | `src/components/MapCanvas.tsx` | `MapCanvas`・`SeriesPanel`・`SeriesDetailCard` | `SeriesLayers`・`SeriesPanel`・`SeriesDetailCard` |
| era スライダーの位置（`eraPosition`） | `src/components/MapCanvas.tsx` | `EraSlider` | `EraSlider`・`SeriesLayers`・`SeriesPanel` |
| エピソードの取得の状態（`EpisodesState`） | `src/components/MapCanvas.tsx` | `MapCanvas` | `SeriesDetailCard` |
