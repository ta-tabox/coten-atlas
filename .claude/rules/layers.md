...
| 一覧パネル全体と、その区画ごとの開閉 | `src/components/SeriesPanel.tsx` の state。パネルの外に開閉を読む部品が無いので、`MapCanvas` へ上げない。区画ごとの開閉も `SeriesSection` でなく `SeriesPanel` に置き、パネル全体を閉じて開き直しても残す |
