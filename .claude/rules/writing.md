...
- 理由は一文まで。
  ...
  ただし ADR の規約5（`docs/adr/README.md`）が ADR にしないとする事柄（決定と読めない事柄・一つのモジュールの内側に閉じた実装の選択）なら、ADR を起こさず、その記述を足すコミットの本文に理由を書く
...
この表が禁止語の正で、`scripts/lint-comments.ts` と `scripts/lint-vocabulary.sh` の `BANNED_WORDS` はどちらもここを機械が読める形へ写したもの（表を直したら2本とも直す）。
前者は TypeScript のコメントを見て、後者は git の追加行とコミット本文と PR 本文を見る。
