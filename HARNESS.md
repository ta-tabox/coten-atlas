...
| **L3 データ** | `data/` の現物が `web/src/lib/schema/` の契約に合っているか | `node scripts/validate-data.ts` |
| **L4 ビルド** | static export が実際に吐けるか | `next build` |
| **L5 スモーク** | 静的成果物が自足しているか（4xx・実行時エラー・canvas の寸法） | ヘッドレスの Chromium で `out/` を開く（`playwright test`） |
| **人間の目視** | 地図の見た目・スライダーの手触り・実機 | 人間が `pnpm dev` で開く |

L0〜L5 は `pnpm check` の一本にまとまっている（下記）。
...
機械層が一つ増えるたびに、それより後ろを指していた ADR の番号は 1 つずつ実態からずれる。
いま食い違っているのは3件で、[ADR-0009](docs/adr/0009-pnpm-check.md) の `L3 ビルド`、[ADR-0014](docs/adr/0014-e2e-offline-smoke.md) の `L5`（人間の目視）と `L3`（ビルド）、[ADR-0016](docs/adr/0016-playwright-runner.md) の `L4`（スモーク）である。
番号で突き合わせず、層の名前で読む。

L3 を層に持つのは、`data/` が型検査の外に居るため。
`tsconfig.json` の `include` は `web/` 配下しか見ないので、人手で書く層（`themes.geojson`・`eras.json`）が契約から外れても L0〜L2 は緑のまま通る。

L4 を層に持つのは static export の性質による。ビルド時にしか壊れない失敗があり、
L0〜L3 だけでは PR が緑のまま公開が落ちる。

L5 を層に持つのは、L4 までがどれも「配信物へ実際に到達できるか」を見ないため。
...
**`pnpm check` の一本**。中身は L0 → L1 → L2 → L3 → L4 → L5 の順で、安いものから落とす。
アプリは `web/` 配下なので（#39）、**打つ場所も `web/` の中**。

```
cd web && pnpm check   # tsc --noEmit → biome ci . → vitest run → validate-data → next build → smoke
```

L3 の `validate` が L2 の後ろに居るのは、検査器がスキーマそのもので、そのスキーマを保証するのが L2 の反例テストだから。
先に置くと、赤が出たときに「データが壊れている」のか「スキーマが壊れている」のかを人間が切り分けることになる。
L4 の前に居るのは、契約から外れたデータで static export を作っても公開できないので、より安い段で落とすため。

L5 のスモークが連鎖の末尾に居るのは、判定の対象が `next build` の出力だから。
その前には置けない。
ブラウザを立てるのは L5 だけで、回すのは Playwright である（[ADR-0016](docs/adr/0016-playwright-runner.md)）。
...
- **Playwright の配信元** — L5 のスモークが立てる Chromium のバイナリ。
...
- **E2E で地図の絵を検証しない**。ヘッドレスでも描画そのものは出るが、絵を判定するには
  実タイルかそのフィクスチャが要る。L5 が守るのは配信物が自足していることまでで、
...
- **実 API を自動テストで叩かない**。RSS もタイルサーバも外部の可用性に依存するので、
  ...
  L5 のスモークも同じで、タイルサーバへの通信は遮断してスタイルだけを合成のもので返す
