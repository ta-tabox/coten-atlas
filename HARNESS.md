...
L0〜L4 は `pnpm check` の一本にまとまる（下記）。
**L5 は close を妨げない**——issue の完了条件は機械判定（L0〜L4）と人間の判定（L5）に
...
1. 指摘を読み、同意できるものを直す。
   指摘は正典ではないので、同意できないものは直さず根拠を添えて返信する
...
- **E2E で地図の絵を検証しない**。ヘッドレスでも描画そのものは出るが、絵を判定するには
  ...
  見た目は L5 の目視に残す（[ADR-0014](docs/adr/0014-e2e-offline-smoke.md)）
