...
## 文章の規約（2026-09-08 配布）
残る文章（コメント・コミット本文・PR・issue への投稿・md）の規律は `.claude/rules/writing.md`「語彙と読み手」節。
独自用語の用語集は fermentary `RULES.md` §7 が原本で、このリポジトリへは複製しない。
用語集に無い比喩語は使わない。言い換えが存在する比喩は装飾なので直叙で書く。
このリポジトリで独自用語を増やすなら、fermentary の用語集へ載せる判断を人間に諮る。
