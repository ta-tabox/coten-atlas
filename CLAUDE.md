...
## セッション開始時にやること
0. **初期化がまだなら**、fermentary（第二マウント）が見えることを確認する
   （`fermentary/RULES.md` が読めるか）。**見えなければ初期化を始めず**、人間に
   「fermentary が並置されていない」と指摘して連携を求める——並置なしで初期化すると
   膜からも台帳からも切断された孤児プロジェクトが生まれる。
   **初期化済みならここで止まらない**——この器は隔離されても動くのが既定で、
   fermentary の並置は手順を引くときと知見を渡すときにだけ要る
1. `NEXT.md` を見て続きを拾う
2. **fermentary が見えていれば**、その `NEXT.md` に自分宛の搬送メモがあれば拾う
3. **fermentary が見えていれば**、その `ATLAS.md`（terrarium 節）の自行を一瞥し、
   status が実態と乖離していたら直す（初回セッション実施・MVP 到達・方針転換などの
   節目を反映。台帳はこのプロジェクトの状態を fermentary へ伝える唯一の口）
   - **見えないセッションは 2・3 を飛ばし、飛ばしたことを一言申告する**——黙って
     飛ばすと「自分宛が無かった」と「読めなかった」が同じ外形になる
