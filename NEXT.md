...
**fermentary へ搬送済み**（`fermentary/NEXT.md` の3行。宛先は fermentary なのでこちらから実行しない）:
gh-review.md へ「起動を絞る口は job 側の `if:` が唯一」を足すか諮る ／
gh-review 雛形の `claude-code-review.yml` が `timeout-minutes` と `concurrency` を持たない ／
gh-review.md「検証の型」へ「ワークフローを直す PR では陽性が出せない」を足すか諮る。
...
**次の実装は #18**（機械層の配布）で、実装レーンはこの一本。
検査器は既存コードへ当てると必ず違反を出すので、`web/src/` が 2 ファイルの今が導入コストの底である。
配布版 `biome.json` は formatter と「相対 import 禁止」を持ち込むので、#2 以降を先に書くと書き直しになる——
**#2 / #3 / #4 は #18 と並行できない**（`web/package.json` でも交差する）。

- 実装: #2〜#6（S1〜S3）。**#18 の後**
- 実装（先に降りた束）: #27（S6・RSS 同期）／ #26（S8・GitHub Pages 配信）
- 器の構成: #41（VISION・人間との対話が本体）
- gh-review: #10（人間・public 化と実地検証）
- 決定レーン（#18 と並行可）: #25（Claude・引用と出典の反映。散文だけなので交差ゼロ）／
  #19（Claude・#3 依存なのでまだ動かない）
- 配布の未達分: #18（Claude・前提は全部解けている）
