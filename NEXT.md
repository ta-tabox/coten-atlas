...
**#13〜#17**（洗い出しで出た人間担当の5本）。いずれも前提が無く、#1 を待たずに着手できる。
うち #13（フィードとシリーズ一覧）と #14（ベースマップの利用条件）は、
#3 と #6 が `links` とスキーマを固定する前に潰さないと書き直しになる。
...
- gh-review: #8（人間・通電）→ #9（Claude・`check.yml` と `claude.yml`。#1 にも依存）→
  #10（人間・public 化と実地検証）
- 検証と決定: #13〜#17（人間・前提なし）。#19（Claude・#3 依存）
- 配布の未達分: #18（Claude・#1 依存）
