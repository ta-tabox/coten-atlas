...
- 2026-08-29 #66 完了（E2E スモークの配線）。
  `pnpm check` が L0〜L4 の5層になり、「ビルドは通るがアセットへ到達できない」形を機械が捕まえる。
  守らせるのは配信物の自足の一点で、判定は同一オリジンへの 4xx / 5xx・実行時エラー・canvas の寸法（[ADR-0014](docs/adr/0014-e2e-offline-smoke.md)）。
  **ブラウザを立てる検証は Playwright が回す**（[ADR-0016](docs/adr/0016-playwright-runner.md)）。
  最初は素の node スクリプトで組んだが、一つのファイルに CLI と import の二つの入口ができて判定の走る場所が読めず、陽性対照を Vitest へ入れた回に L2 までブラウザを立て始めたので寄せ直した。
  新しいテストの置き場は「ブラウザが要るか」で決まる（要るなら `tests/e2e/*.spec.ts`、要らないなら `tests/*.test.ts`）。
  **人間の目視は層番号を持たなくなった**。番号は `pnpm check` の連鎖の中の位置を指すもので、目視はその連鎖に居ない。
  機械層が増えるたびに末尾がずれて他の文書の参照まで腐るのを、L4 を挟んだ回に実際に踏んだ。
  レビューで実際の脆弱性が1件出た——配信サーバがパストラバーサルを許し全インターフェースへ bind していた（`/etc/hosts` が 200 で返ることを実測）。
  境界の判定は `resolveWithinRoot` が持ち、テストで固定してある。
  ブラウザのバイナリは `pnpm check` が取りに行かないので、手元では `pnpm exec playwright install chromium` を一度だけ打つ。
