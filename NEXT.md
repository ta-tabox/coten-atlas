...
**次の実装は #2**（S1・ベースマップの全画面表示）。
#18 で機械層が入り、実装を待たせていた前提は解けた。

- 実装: #2〜#6（S1〜S3）。**次はここ**。
  **#5 は #19 が決まるまで書けない**（読み込み口が #19 の決定に従う）
- 実装（先に降りた束）: #27（S6・RSS 同期）／ #26（S8・GitHub Pages 配信）
- 器の構成: #41（VISION・人間との対話が本体）
- 公開の前提: #55（人間 + Claude・ライセンスの決定）。#10 の前提へ入れてある。
  README（#54）は置いた（下記「済んだもの」）
- gh-review: #10（人間・public 化と実地検証）
- 決定レーン: #25（Claude・引用と出典の反映。散文だけなので交差ゼロ）／
  #19（Claude・#3 依存。#5 のブロッカーでもある）
...
- 2026-08-27 台帳の乖離を5点直し、公開の前提を2枚起票した。
  ...
  食い違っていたので現行の `ARCHITECTURE.md` へ揃えた（#25 自体は未着手のまま）。
