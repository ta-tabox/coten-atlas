...
そのうえ `feed/` と `schema/` が既にディレクトリなので、同じ `lib/` に二つの割り方が並ぶ。
...
- **`schema/` を領域（episode・era・series・locus）と部品（geojson・link・text）に割る。**
  層が混ざっているのは事実だが、部品の置き場に付けられる名前が無い。
...
- **テストを実装から分け、`schema/` の混み具合をそちらで落とす。**
  `schema/` が混んでいるのは実装とテストが同じ階層に並ぶからなので、テストを分ければ部品の置き場に名前を付けずに済む。
  ただし `schema/` だけ分けるとリポジトリの中に規約が二本並び、この ADR が地図まわりで消したばかりの非対称と同じ形になる。
  リポジトリ全体へ当てるなら `ARCHITECTURE.md` §6 の `tests/` の定義ごと引き直すことになり、`lib/` の割り方という枠を超える。
  採らなかったのではなく分けたので、#129（テストの置き場をリポジトリ全体で見直すか決める）が引き取る
- **`base-path.ts` を動かす。**
...
- `schema/` の混み具合は残る。
  部品の置き場に名前が付くのは、その区分が実体を持ってからになる

## 覆る条件

`schema/` の部品に固有名が付いたとき。
外部仕様の写し（GeoJSON）と、このリポジトリが決めた共有の制約（`text`・`link`）が別々の理由で増え始めれば、汎用語に頼らない名前でそれぞれを名指せる。
