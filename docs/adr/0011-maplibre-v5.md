# 0011. maplibre-gl は v5 系に固定する

- **状態**: supersede 済み（→ 0012）
- **決定日**: 2026-08-27
...
- 依存を上げるときは major を跨がせない。跨ぐなら worker の配布をこの器が引き受ける
- 画面が「attribution だけ乗った空白」になったら、まず worker の取得が 404 になっていないかを疑う

## 覆る条件

react-map-gl か Next.js の側が v6 の worker を解決するようになったとき。
または、この器が worker の配布を引き受けると決めたとき（globe 投影のように v6 でしか得られないものが要るようになった場合）。
