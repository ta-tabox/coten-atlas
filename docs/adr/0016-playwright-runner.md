...
spec は `web/tests/e2e/` に置き、設定は `web/playwright.config.ts` が持つ。
...
`pnpm smoke` が `playwright test` を呼び、連鎖の末尾に居る。

## 理由

**入口が一つになる。** `main()` と argv の分岐が消えるので、「どこで判定が走るのか」がファイルの構造から読める。

**層の境界が戻る。** ブラウザを立てるのは L4 だけになり、L2 は jsdom へ戻る。実測でも、Chromium を見えない場所へ向けて `vitest run` を回すと 54 本すべてが通る。
...
- **runner が二つになる。** 新しいテストを書くときの分かれ目は「ブラウザが要るか」の一点で、要るなら `tests/e2e/*.spec.ts`、要らないなら `tests/*.test.ts` に置く
- Vitest の既定は `*.spec.ts` も拾うので、`vitest.config.ts` の `exclude` が `tests/e2e/**` を外している。外さないと同じ spec を二重に走らせる
