...
4. **書き換えない。** 覆すときは新しい ADR を書き、旧 ADR のヘッダに
   ...
   **意味が変わる変更は、一部だけでも同じ**。
   ...
   - この例外はこのリポジトリが先に開き、2026-09-06 に雛形（fermentary）へ取り込まれた。適用の記録は次の二度。
...
6. **「決定」の節は繰り返し参照される前提で書く。**
   ADR には二つの読まれ方がある。
   なぜそう決めたかを一度読む読み方と、何を書けばよいかを繰り返し参照する読み方で、後者が当たるのは「決定」の節だけである。
   だから決定の節は、決定した順でなく**読み手が参照する単位**（欄ごと・値ごと・状況ごと）で並べる。
   決定に番号を振って節を割ると、一つの欄を調べる人が複数の節を読むことになる。
   - **値の一覧・状況と選択の対応・可否の別は表にする。**
     散文より短く、目で走査できる
   - **採らなかった案は「案 ／ 採らなかった理由」の 1 行 1 案にする。**
     箇条書きで 1 案 3 行になると、案が何件あるかが見えなくなる
   - **理由は 1 項目 1 行にする**
   - **文脈は決定を読むために要る事実だけにする。**
     決定が出た後に読む必要のない経緯は書かない
   - **既にマージされたレコードをこの形へ組み直すのは規約4 に当たる。**
     当てるのは新しく書くレコードと、まだマージされていないレコードだけである
   - 適用の記録: [0034](0034-series-vocabulary.md) を 2026-09-10 に組み直した（8,391 字 → 5,710 字）。
     決定の中身は動かしておらず、マージ前だったので規約4 に触れていない
