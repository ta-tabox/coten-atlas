...
1. **1ファイル1決定**。命名は `NNNN-<slug>.md`（4桁ゼロ埋め・連番・欠番を作らない）
   並行する PR が同じ番号を取ると後にマージする側で番号が重なるので、番号は main の `docs/adr/` と、開いている PR が足す ADR の両方を見て、空いている最小の連番を取る。
   番号を取った PR がマージされずに閉じたら、その後に番号を取った PR の番号を詰める
...
5. **「決定」と読めない事柄は ADR にしない。**
   ...
   **一つのモジュールの内側に閉じた実装の選択も ADR にしない。**
   モジュールの内側の選択は、そのモジュールを直す人がコードとコミットの履歴から辿れるので、ADR に置くと決定の一覧にアーキテクチャの水準でない行が混ざる。
   内側に閉じるかは、その選択がモジュールの外から見える形（公開する関数と型・props・データの形・層の依存の向き・他のモジュールが置く前提）を変えるかで判じる。
   閉じるなら、理由はコミットの本文と PR の本文に書き、コードのコメントには 2 文までの理由を置く。
