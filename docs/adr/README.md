   ...
   **意味が変わる変更は、一部だけでも同じ**。
   ...
   - この例外はこのリポジトリが先に開き、2026-09-06 に雛形（fermentary）へ取り込まれた。適用の記録は次の二度。
     ...
     - 0016・0018・0019・0020・0024 に 2026-09-02 に適用した。
       #98（「綴り」の転用を層ごとの専用語へ振る）がリポジトリの文章から外した転用が ADR に 9 箇所残っていたので、同じ割り当てでファイル名・表記・文字列・名前・ファイルの形・写しへ置き換えた。
       残したままにすると、ADR を読んで書く側が同じ転用を再生産する。
       PR #112（位置情報の段階分けとシリーズ・事物の分離）で実際に起きた
     - 0025・0028・0031・0032・0033 と本書に 2026-09-10 に適用した。
       雛形が「器」を廃した比喩語にしたので、`リポジトリ` へ置き換えた。
       欄の意味が変わらない箇所は指す先の名前にした（`NEXT.md`・`lib/`・`catalog/`）
...
6. **簡潔に書く。**
   ...
   - 適用: [0034](0034-series-vocabulary.md) を 2026-09-10 に組み直した（8,391 字 → 5,710 字）。
     マージ前なので規約4 に触れていない
