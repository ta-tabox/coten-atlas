...
| [0045](0045-no-layer-number-for-human-review.md) | 検証の層の番号は `pnpm check` の連鎖の位置だけを表し、人間の目視は番号を持たない | 2026-08-29 | 採用 |
| [0046](0046-catalog-check-in-unit-layer.md) | `catalog/` の検査は独立した層にせず、L2 のユニットに置く | 2026-08-30 | 採用 |

**0014 は人間の目視を L5 と呼んでいる。**
検証の層の番号を [0045](0045-no-layer-number-for-human-review.md) で `pnpm check` の連鎖の位置に限り、人間の目視から番号を外したので、0014 の L5 は「人間の目視」と読む。
