...
4. **書き換えない。** 覆すときは新しい ADR を書き、旧 ADR のヘッダに
   ...
   - 例外は語彙の置き換えに限って二度適用した。
     どちらも動かしたのは語彙だけで、決定・理由・帰結は動かしていない
     - [0017](0017-local-only-instructions.md) に 2026-08-29 に適用した。
       公開に先立ち、外から読んで解けない語彙を全 ADR の本文で置き換えた
     - 0016・0018・0019・0020・0024 に 2026-09-02 に適用した。
       #98（「綴り」の転用を層ごとの専用語へ振る）が器の文章から外した転用が ADR に 9 箇所残っていたので、同じ割り当てでファイル名・表記・文字列・名前・ファイルの形・写しへ置き換えた。
