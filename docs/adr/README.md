...
4. **書き換えない。** 覆すときは新しい ADR を書き、旧 ADR のヘッダに
   ...
   - 例外は語彙の置き換えに限って二度適用した。
     ...
     - [0017](0017-local-only-instructions.md)、2026-08-29。公開に先立ち、外から読んで解けない語彙を全 ADR の本文で置き換えた
     - 0016・0018・0019・0020・0024、2026-09-02。#98 が器の文章から外した「綴り」の転用（ファイル名・表記・文字列・名前・ファイルの形の意味で使っていたもの）を、ADR に残っていた 8 箇所でも同じ割り当てで置き換えた。
       残したままにすると、ADR を読んで書く側が同じ転用を再生産する（PR #112 で実際に起きた）
