...
歴史側が典拠について指示されているのは次の 5 つで、実体はワークフローの `--append-system-prompt` にある。

| 指示 | 理由 |
|---|---|
| 座標は Pleiades（古代）・GeoNames（近現代）・Wikidata に限る | `latitude.to` と `geodatos.net` はスクレイプ由来の集約サイトで、誤りが混じっても検出できない |
| 典拠の URL はその行の主張を直接含むページに限る | 初回実走で、彭城の座標に鹿邑県の記事の URL が付いた |
| 各行に典拠の格を書く（一次 / 二次 / 三次 / 参考程度） | 格を書かせないと、事典と集約サイトが同じ重みで並ぶ |
| Wikipedia は脚注の文献へ辿る。辿れなければ Wikipedia と明記する | Wikipedia は索引として使えるが、それ自体は三次である |
| 日本・中国・朝鮮半島の主題は日本語の典拠にも当たる | 66 件のうち日本のシリーズが 14 件ある。初回実走は英語の検索だけで走った |
