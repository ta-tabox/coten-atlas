...
**配信の後に走る検査もこの表に居ない。** 相手が配信された実物なので `pnpm check` の連鎖に置けず、番号を与えると連鎖の続きに見える。
中身は「2. 判定の口」の「配信の後に走るもの」が持つ。
...
**`deploy.yml` の deploy ジョブの後段が、配信された実物を一度開く**（[ADR-0036](adr/0036-post-deploy-reachability.md)）。
見るのは、配信されたページが名指している参照へその URL から到達できるかの一点だけである。

L4 の遮断版スモークは `out/` を `BASE_PATH` の下へ配信して開くので、アプリが焼いた接頭辞と検査が配信する接頭辞が同じ定数から来る。
接頭辞そのものが配信先と合っていない形はそこを素通りするので、実際に配れた先から引いて確かめる。
...
ブラウザは立てない。
canvas と実行時エラーは L4 が同じバイト列に対して見ているので、配信で新しく入る変数である到達性だけを見る。
...
- **実 API を自動テストで叩かない**。RSS もタイルサーバも外部の可用性に依存するので、
  ...
  配信の後に走る検査（[ADR-0036](adr/0036-post-deploy-reachability.md)）はこの則の外に居る。
  判定の対象が配信された実物そのものなので、そこへ到達できないことは外部の都合ではなく、この検査が探している事故そのものに当たる
