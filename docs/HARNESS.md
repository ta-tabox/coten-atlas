...
**配信の後に走る検査もこの表に載せない。** 検証の対象が配信された実物なので `pnpm check` の連鎖に置けず、番号を与えると連鎖の続きに見える。
内容は「2. 判定の口」の「配信の後に走るもの」が持つ。
...
**`deploy.yml` の deploy ジョブの後段が、配信されたページを一度取得する**（[ADR-0036](adr/0036-post-deploy-reachability.md)）。
検証するのは、配信されたページが名指している参照へ、そのページが配信された URL から到達できるかの一点だけである。

L4 の遮断版スモークは `out/` を `BASE_PATH` の下へ配信して取得するので、アプリがビルド時に埋めた接頭辞と検査が配信する接頭辞が同じ定数から来る。
接頭辞そのものが配信先と合っていない形は L4 を通過するので、実際に配信された URL から取得して検証する。

**配信の後に走る検査は、閉じてよいかの判定には使わない。**
走るのが main へ入った後なので、赤が出ても close の判定には間に合わない。
「これが緑なら閉じてよい」と言えるのは変わらず `pnpm check` だけである。

ブラウザは立てない。
canvas と実行時エラーは L4 が同じバイト列に対して検証しているので、配信で新しく増える条件である到達性だけを検証する。
...
- **実 API を自動テストで叩かない**。RSS もタイルサーバも外部の可用性に依存するので、
  ...
  配信の後に走る検査（[ADR-0036](adr/0036-post-deploy-reachability.md)）はこの規則の対象外とする。
