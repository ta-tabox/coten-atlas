...
`@historian` は格を参考程度と報告したが、[ADR-0037](../adr/0037-sources-layer.md) の定義で地名辞典の値を持つデータベースは三次に当たるので、三次にした。
現在の値は典拠の値から約 1.5km 以内に在る。

代表点を旅順に置いたのは、第 10・12・13 回が旅順の攻防を扱い、陸の主な舞台の満洲の中で最も多くの回が割かれているためである。
...
事物の `id` は、[ADR-0034](../adr/0034-series-vocabulary.md) の事物の `id` の表の「中国の地名は声調記号を付けないピンインで書く」を当てて `lushun` にした。
...
ADR-0034 の同じ行の「英語の文献で通用する表記を使う」を先に当てれば `port-arthur` になり、二つの条項のどちらを優先するかは規則の判断なので、#170（代表点に選んだ時代の地点が複数の名を持つとき、事物の `id` にどの名を使うかを決める）へコメントで足した。

## `region`・`kind`・`title`

`region: 中国` は、各回が朝鮮半島と日本も扱って区画が跨るので ADR-0034 の `region` の選び方の表の 2 行目に当て、出来事の本拠の主な舞台（満洲）で決めた。
ADR-0034 の継ぎ目の表は、満洲を `中国` に入れる。
...
| 論点 | 仮決定 | 覆りうる根拠 |
|---|---|---|
| 代表点 | 旅順 | 戦いの規模で数えれば、最大の陸戦の奉天になる |
| 事物の `id` | `lushun` | #170 で英語の文献の慣用を中国の地名の条項より先に当てると決まれば `port-arthur` になる |
| `region` | `中国` | 日本の国家の歩みとして読む読み手には `日本` が自然だが、ADR-0034 の出来事の本拠（主な舞台）の基準を変えない限り値は変わらない |
| `id` の区切り | `nichirosenso` | 辞書の見出しの `‐` の位置で割れば `nichiro-senso` になる |
