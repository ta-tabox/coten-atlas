...
代表点をデイトンに置いたのは、二人が自転車店と飛行機の工房を構えた地で、オーヴィルの生地と没地でもあるためである。
...
カタカナの部分だけ、ADR-0034 の `id` の表の「カタカナの外来固有名」の行で `wright` にした。
...
| 論点 | 仮決定 | 覆りうる根拠 |
|---|---|---|
| `id` の区切り | `wrightkyodai` | 辞書の見出しの `‐` の位置で割るか、カタカナの部分と漢字の部分を別の語と見れば `wright-kyodai` になる |
