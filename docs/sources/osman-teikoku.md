...
1453 年から帝国の終わりまでの都で、各回の山場がコンスタンティノープルの攻略なので、ここにした。
...
`region: 西アジア` は、版図がバルカンと北アフリカに跨るので、ADR-0034 の `region` の選び方の表の 2 行目で本拠の区画にした。
種別 `集団` の本拠は発祥の地で、発祥の地のソユトはアナトリアに在り、ADR-0034 の継ぎ目の表でアナトリアは `西アジア` である。
...
| 代表点 | `constantinople` | 各回の前半を重く見ればブルサかエディルネになる（#171） |
