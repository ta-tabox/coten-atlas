...
値は変わらない。
2026-09-11 に人間が仮決定を採用して決着させ、見直しを [#180](https://github.com/ta-tabox/coten-atlas/issues/180)（ムガール帝国（mughal-teikoku）の代表点をアグラとデリーのどちらにし、region をどうするか）へ切り出した。

| 論点 | 仮決定 | 覆りうる根拠 |
|---|---|---|
| `end` | 1857（最後の皇帝の廃位） | 流刑の 1858 年を終わりとする数え方がある |
| `region` | `南アジア` | 王家の出自のフェルガナを発祥の地と見れば `中央ユーラシア` になる |
| 代表点 | `agra`（アグラ） | 事績の中心をアウラングゼーブの治世とインド大反乱（第 9・12 回）に置けばデリーになる。見直しは #180 で行う |
...
| [#180（見直し）](https://github.com/ta-tabox/coten-atlas/issues/180) | 採用した仮決定を人間が見直す論点と案 |
