...
舞台がパリとヴェルサイユに分かれるので、ADR-0041 の決定（#171）により、番組が扱う主な事の運びが起きた舞台を中心の場所とした。
各回の第 5〜8 回が扱うバスティーユ牢獄の襲撃・国王の処刑・恐怖政治がパリで起きたので、パリを主な舞台とした。

並行して開いた #105（20 世紀）の PR #168 も、`levi-strauss`（レヴィ＝ストロース）の代表点に事物の `id` `paris` を使っていた。
ADR-0041 の決定（#166）の同じ地名の表の順 1 により、種別が `出来事` のこのシリーズが `paris` を使い、種別が `人物` の `levi-strauss` が具体的な地点名へ移る。
...
`region: ヨーロッパ` は、舞台のフランスと戦った相手の国々がヨーロッパに収まるので、ADR-0041 の `region` の選び方の表の 1 行目で決めた。
...
同じ日に人間が #166 と #171 で規則を決め、Claude がその規則で事物の `id` と代表点を当て直した。
値は変わらない。
2026-09-11 に人間が仮決定を採用して決着させた。
2026-09-12 に人間が、規則と各回の配分で決まった値に残る別の候補も優先度を下げて見直すと決め、見直しを [#198](https://github.com/ta-tabox/coten-atlas/issues/198)（フランス革命（france-kakumei）の代表点をパリとヴェルサイユのどちらにするか）へ切り出した。

| 論点 | 仮決定 | 覆りうる根拠 |
|---|---|---|
| `start` | 1789（バスティーユ牢獄の襲撃） | 名士会の招集（1787）から数える区切りを Britannica の概説が示す |
| 代表点 | `paris` | 事の運びの中心を三部会と球戯場の誓い（第 4 回）に置けばヴェルサイユになる。見直しは #198 で行う |
...
| [#198（見直し）](https://github.com/ta-tabox/coten-atlas/issues/198) | 採用した仮決定を人間が見直す論点と案 |
