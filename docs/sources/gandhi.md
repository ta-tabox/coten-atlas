...
`title: ガンディー` に指摘は出なかった。

## 仮決定と論点

2026-09-11 に Claude が `@historian` の結果を見て仮決定し、同日に #171 の決定（ADR-0041）で代表点の理由を事績の中心として当て直した。
値は変わらなかった。
2026-09-11 に人間が仮決定を採用して決着させ、見直しを [#181](https://github.com/ta-tabox/coten-atlas/issues/181)（ガンディー（gandhi）の代表点をアフマダーバードとヨハネスブルグのどちらにするか）へ切り出した。

| 論点 | 仮決定 | 覆りうる根拠 |
|---|---|---|
| 代表点 | アフマダーバード（事績の中心） | 第 3・4 回の南アフリカでのサティヤーグラハを主な事績と見れば、サティヤーグラハを始めたヨハネスブルグになる。期間で数えれば、南アフリカ（1893〜1914 年の約 21 年）がアフマダーバード（約 13 年）より長い。見直しは #181 で行う |
...
| [#181（見直し）](https://github.com/ta-tabox/coten-atlas/issues/181) | 採用した仮決定を人間が見直す論点と案 |
