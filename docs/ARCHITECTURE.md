...
- 全画面マップ + 下部に era スライダー（era 名を等幅のセルに並べ、下に区間の境目の年を置く。現在窓の年の範囲を、文字とセルの上の帯で示す）。
  ...
  現在窓と重ならない事物は地図に描かない
- 左に開閉パネル: 現在窓で地図に円が描かれているシリーズの一覧。
  パネルでクリックしたシリーズを選択し、代表点を持つならその代表点へ `flyTo` する。
  地図のクリックとパネルのクリックは単一の selection state を書き、選択中のシリーズは地図で輪に囲まれ、パネルで強調される
- 位置なしのシリーズ（`anchor` が `ANCHOR_UNLOCATED`）は地図に出ない。
  同じパネルの別区画に、地図と区別し、現在窓で絞らずに全件を並べる。
  パネルの年代の欄は、時期なしのシリーズには出さない（理由は [ADR-0046](adr/0046-series-panel-unlocated-section.md)）
