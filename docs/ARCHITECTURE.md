...
- `region` の 13 値・種別が最低 1 つ在ること・`tags` の上限・era 級の時代名の禁止・`title` が `ショート` と `ジンブンガク` で始まらないこと・`kind: place` と `ANCHOR_UNLOCATED` を組まないことは、`web/src/lib/schema/series.ts` が検査する。
