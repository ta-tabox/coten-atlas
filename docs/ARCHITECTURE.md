...
シリーズと事物は 1 対多で、多の側（事物）が `seriesId` で一の側を指す。
位置情報は二段階で持つ。
...
[
  {
    ...
    "summary": "",         // 自前の要約を入れる欄。番組の説明文は引用しないので当面は空
    "region": "ヨーロッパ",
    "season": 2,           // 割当キー。itunes:season の値
  ...
  },
  {
    ...
    "timeRange": "untimed",  // 時期なし。コードは TIME_RANGE_UNTIMED の名で読む。置けるのは種別が概念史だけの位置なしのシリーズに限る
    "summary": "",
    "region": "地域なし",
    "season": 7,
    "links": [],
  ...
  "features": [
    {
      ...
      "properties": {
        "id": "sparta-city",       // 鍵。属性は series.json が持つ
...
`series.json` の例の `2` はスパルタ、`7` は世界三大宗教の、フィードでの `itunes:season` の値である。
...
- `anchor` は代表点の事物 id か、位置なしを表す `"unlocated"` のどちらかで、null を使わない
  null は「まだ置いていない」と「置かないと決めた」を語らない
...
- シリーズの `timeRange` は年の閉区間か、時期を持たないことを表す `"untimed"` のどちらか（理由は [ADR-0039](adr/0039-untimed-concept-series.md)）
...
- 地図に出すか一覧にだけ出すかは `anchor` だけで決まり、シリーズは描き分けの欄を持たない
  ...
  代わりの点であることと選んだ理由は、`docs/sources/<シリーズ id>.md` の「代表点」節が持つ（描き分けない理由は [ADR-0042](adr/0042-drop-series-kind.md)）
- `region` と `tags` の消費者は §4「シリーズの近接」（関連シリーズ行と tag 絞り込み）である
  近接のためにスキーマを増やさないので、この二つが判定の材料になる
- `region` は陸地を重ならないように割った 12 区画と `地域なし` の 13 値で、区画を一つ選ぶと嘘になるシリーズが `地域なし` を書く（閉じた集合にする理由は [ADR-0044](adr/0044-series-vocabulary-without-kind.md)）
...
- **終わっていない era の `end` には年を書かず `"present"` を置く**（理由は [ADR-0019](adr/0019-era-open-end.md)）
  置けるのは末尾だけで、この区間を補間するときの右端はビルドした時点の年である（ブラウザの時計を読まない理由は [ADR-0043](adr/0043-era-fade-window-only.md)）
...
- 現在窓の幅は era 空間の 1 区間の半分（`WINDOW_WIDTH_IN_ERAS`）で、窓の両端を年へ変換してから `timeRange` と比べる
  重なり率は重なる年数を窓と `timeRange` のうち短い方の年数で割り、smoothstep に通して濃さにする（理由は [ADR-0038](adr/0038-era-space-window.md)）
...
- 全画面マップ + 下部に era スライダー（era 名を等幅のセルに並べ、下に区間の境目の年を置く。現在窓の年の範囲を、文字とセルの上の帯で示す）
  ...
  現在窓と重ならない事物は地図に描かない
- 左に開閉パネル: 現在窓で地図に出ているシリーズと、位置なしのシリーズを、別々の区画に並べる一覧
- 位置なしのシリーズ（`anchor` が `ANCHOR_UNLOCATED`）は地図に出ず、パネルの別区画にだけ出る
- 第二段階のデータが入った後は、代表点と精緻な事物のどちらを描くかを利用者が画面で切り替える
...
- **tag 絞り込み**: パネルに並ぶシリーズ（現在窓で地図に出ているシリーズと位置なしのシリーズ）の tags を並べ、選んだタグ一つを持つシリーズだけを地図とパネルに残す
  filter state は selection と別の state で、絞り込みで選択中のシリーズが地図とパネルから消えても選択は外れない

近接のためにスキーマは増やさない。
判定は既存の properties（`tags` / `region` / `timeRange`）だけで行う。
シリーズ間の明示的な関連リンク（`related` のような属性）は、全シリーズ（67 件）へ人手で張る費用が S7 に乗るので採らない。
`tags` も `region` も他のシリーズと共有しないシリーズでは関連シリーズが 0 件になるので、0 件なら行ごと出さない。

## 5. RSS 同期パイプライン

- feedUrl: `https://anchor.fm/s/8c2088c/podcast/rss`
  Apple Podcasts lookup API `https://itunes.apple.com/lookup?id=1450522865` が返す `feedUrl` の値で、同期は lookup API を経由せずこの URL を直接叩く
- フィードの形（パーサはこれを前提にしてよい）:
...
このリポジトリは `catalog/`（人間キュレーション層）と RSS 同期スクリプトを持つので、
...
ルートに残るのは、道具がその位置を要求するものだけである（理由は [ADR-0032](adr/0032-docs-under-docs.md)）。

まだ存在しないもの: `docs/VISION.md`（#41）。
`docs/sources/` はシリーズごとに 1 ファイルを持つ。
ファイルの在るシリーズは裏どりが済んでおり、ファイルの無いシリーズは未調査である。
...
- **番組の説明文・ロゴ・カバーアートを使わない**
  載せるのはシリーズ名とエピソードタイトルだけ
- **公開サイトにユーザ登録・コメント等の動的機能を持たない**。static export の前提から外れる
  データの編集は手元の管理画面が担い、公開サイトの成果物に含まれない
- **第一段階で Point 以外の図形を持たない**。版図や経路の精緻な図形は第二段階（S9）で、完了条件の外
...
- 一つのエピソードが複数シリーズに跨る回（対談・番外編）の見せ方
  `seriesId` は単数 nullable で、割当キーが season なので、season を持つ回は必ず一つのシリーズへ入る
  season を持たない回（番外編・特別編）はどのシリーズにも割り当たらず、詳細カードはシリーズに割り当たった回だけを出す
  番外編・特別編を詳細カードに出すと決めたときに、どのシリーズの下に見せるかを決める
- フッタの物理的な置き場所（画面下端の常設か、スライダー脇の小さな帰属＋リンクか）
  全画面マップの上で一覧パネル・era スライダー・詳細カードと置き場を争うので、#87（フッタに番組公式と /about への導線を置く）に着手するときに決める
- モバイルでの振る舞い（全画面マップ + 下部スライダー + 左パネル）の範囲は #254（スマホでの画面構成の範囲を決める）に着手するときに決める
- 概念史以外で現在まで続く主題の `end` をどう書くか（era の `end` に置く `"present"` をシリーズにも許すか）
  当てはまるシリーズが無いので、現物が出たときに決める
