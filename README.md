...
確定事項の一覧とそれぞれの根拠は [ARCHITECTURE.md](ARCHITECTURE.md) の「技術スタック（確定事項）」が持つ。
...
- [HARNESS.md](HARNESS.md) — 何をもって「動いた」と言うか。開発の入口（判定の口の打ち方）もここ
...
地図と時代区分の上への整理はこの器が独自に行ったものであって、番組の見解ではない。
