/**
 * `catalog/` の現物へ読み込み口が届くかを見る。
 *
 * 現物がスキーマに合っているかは `tests/catalog.test.ts` が持つので、ここが見るのは在り処の解決だけである。
 * `catalog/` は `web/` の外にあり、ここが唯一その位置を知っている場所なので、置き場が動けば型でも lint でも赤くならないまま実行時に落ちる。
 */
